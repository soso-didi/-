...
function webViewerLoad() {
  ...
  const clearMarkupButton = document.getElementById("clearMarkupButton");
  let annotationEditorUIManager = null;
  clearMarkupButton?.addEventListener("click", () => {
    if (!annotationEditorUIManager) {
      return;
    }
    const editors = [];
    for (let pageIndex = 0; pageIndex < PDFViewerApplication.pdfViewer.pagesCount; pageIndex++) {
      for (const editor of annotationEditorUIManager.getEditors(pageIndex)) {
        if (editor.editorType === "highlight" || editor.editorType === "ink") {
          editors.push(editor);
        }
      }
    }
  ...
  PDFViewerApplication.initializedPromise.then(() => {
    const resetInitialHorizontalScroll = () => {
      config.viewerContainer.scrollLeft = 0;
    };
    const resetAfterInitialLayout = () => {
      resetInitialHorizontalScroll();
      requestAnimationFrame(resetInitialHorizontalScroll);
      setTimeout(resetInitialHorizontalScroll, 250);
      setTimeout(resetInitialHorizontalScroll, 1000);
    };
    PDFViewerApplication.eventBus._on("documentinit", resetAfterInitialLayout, {
      once: true
    });
    PDFViewerApplication.eventBus._on("pagerendered", resetAfterInitialLayout, {
      once: true
    });
    PDFViewerApplication.eventBus.on("annotationeditoruimanager", ({
      uiManager
    }) => {
      annotationEditorUIManager = uiManager;
      clearMarkupButton.disabled = false;
    });
  });
}
